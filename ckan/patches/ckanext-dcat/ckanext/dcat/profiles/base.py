...
class RDFProfile(object):
    ...
    def _license(self, dataset_ref):
        ...
        for distribution in self._distributions(dataset_ref):
            ...
            if license:
                log.debug('In base.py licenza trovata nella dist %s', license)
    ...
    def _add_list_triple(
        ...
        #log.debug('items in _add_list_triple %s %s %s',items,subject, predicate)
        for item in items:
            # ensure URIRef items are preprocessed (space removal/url encoding)
            if _type == URIRef:
                _type = CleanedURIRef
            if _datatype:
                ...
                object = _type(item)
                if 'theme' in predicate:
                 if 'data-theme' not in object:
                  object=object.replace('["','')
                  object=object.replace('"]','')
                  object='http://publications.europa.eu/resource/authority/data-theme/'+object
                  self.g.add((subject, predicate, URIRef(object)))
                 else:
                  self.g.add((subject, predicate, object))
                elif 'language' in predicate:
                 if 'authority' not in object:
                  object=object.replace('["','')
                  object=object.replace('"]','')
                  object='http://publications.europa.eu/resource/authority/language/'+object
                  self.g.add((subject, predicate, URIRef(object)))
                 else:
                  self.g.add((subject, predicate, object))
                else:
