...
def setup_db():
    ...
    for t in (package_multilang_table,
        ...
        # SQLAlchemy 2.0 (CKAN >= 2.12): niente metadata "bound", engine esplicito
        engine = meta.engine or Session.get_bind()
        if inspect(engine).has_table(t.name):
            ...
            try:
                ...
                if inspect(engine).has_table(t.name):
                    Session.execute(text(f'DROP TABLE {t.name}'))
                    Session.commit()
                raise e
...
_mapper_registry.map_imperatively(PackageMultilang, package_multilang_table)
