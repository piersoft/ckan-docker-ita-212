...
def ensure_sqlalchemy_url_in_ini():
    conn_str = os.environ.get("CKAN_SQLALCHEMY_URL")
    if not conn_str:
        print("[prerun] CKAN_SQLALCHEMY_URL not defined, not updating sqlalchemy.url")
        return

    print(f"[prerun] Forcing sqlalchemy.url in {ckan_ini} from CKAN_SQLALCHEMY_URL")
    cmd = ["ckan", "config-tool", ckan_ini, f"sqlalchemy.url = {conn_str}"]
...
def init_db(retry=None):

    if retry is None:
        retry = RETRY
    elif retry == 0:
        print("[prerun] Giving up after retries on db init")
        sys.exit(1)

    db_command = ["ckan", "-c", ckan_ini, "db", "init"]
    print(f"[prerun] Initializing or upgrading db - start (tries left: {retry})")
    ...
    try:
        ...
        if "OperationalError" in output:
            print(output)
            print("[prerun] Database not ready, retrying in 10s...")
            time.sleep(10)
            return init_db(retry=retry - 1)
